...
export default function EmployeesPage() {
  ...
  const [alertData, setAlertData] = useState<{ title: string; msg: React.ReactNode; variant: 'success' | 'error' | 'info' } | null>(null);
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAlertData({
        ...
        msg: (
          <div style={{ textAlign: 'center' }}>
            <p>Please give this temporary PIN to the employee:</p>
            <div style={{ fontSize: 32, fontWeight: 800, letterSpacing: 4, margin: '16px 0', color: 'var(--success)' }}>
              {response.data.pin}
            </div>
            <p style={{ fontSize: 13, color: 'var(--text-secondary)' }}>They must enter this PIN in the mobile app to create a new password.</p>
          </div>
        ),
        variant: 'success'
      });
      ...
      const msg = err.response?.data?.message;
      setAlertData({ title: 'Error', msg: Array.isArray(msg) ? msg.join(', ') : msg ?? 'Failed to request password reset.', variant: 'error' });
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleStatusToggle = async (id: string, newStatus: string) => {
    try {
      ...
      const msg = err.response?.data?.message;
      setAlertData({ title: 'Error', msg: Array.isArray(msg) ? msg.join(', ') : msg ?? 'Failed to update status.', variant: 'error' });
    }
  };
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      const msg = err.response?.data?.message;
      setAlertData({ title: 'Error', msg: Array.isArray(msg) ? msg.join(', ') : msg ?? 'Failed to delete employee.', variant: 'error' });
    }
  };
  ...
  return (
    <>
      ...
      <Dialog
        ...
        title={editingId ? 'Edit Employee' : 'Register New Employee'}
        maxWidth={600}
        footer={
          <>
      ...
      <Dialog
        ...
        footer={
          <>
            <button type="button" className="btn btn-ghost" onClick={() => { setResetPasswordConfirm(null); setAdminPasswordValue(''); }}>Cancel</button>
            <button type="submit" form="reset-pwd-form" className="btn btn-primary" disabled={isSubmitting || !adminPasswordValue}>
